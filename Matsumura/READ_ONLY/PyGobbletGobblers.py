...
class GobbletGobblers():
    ...
    def getPieces(self,side):
        # 指定サイドのエラーチェック
        if side not in [0,1]:
            logger.debug("sideで[0,1]以外を指定した [{}]".format(side))
            return None       
    ...
    def putPiece(self,side,size,pos):        
        ...
        if size not in [0,1,2]:
            logger.debug("sizeを[0,1,2]以外を指定した [{}]".format(size))
            return False

        # 指定位置のエラーチェック
        if pos not in range(9):
            logger.debug("posを[0〜8]以外を指定した [{}]".format(pos))
            return False    
        
        # 持ち駒に含まれているかのチェック
        if size not in self.getPieces(side):
            logger.debug("そのサイズは持ち駒にありません [side : / size : / pieces : {}]".format(side,size,self.getPieces(side)))
            return False

        # おける位置かどうかのチェック
        if not self.chk_pos(size,pos):
            logger.debug("そのサイズのコマを置けない位置です [size: {} / pos {}]".format(size,pos))            
            return False
    ...
    def movePiece(self,side,fromPos,toPos):        
        ...
        if fromPos not in range(9):
            logger.debug("fromPos[0〜8]以外を指定した [{}]".format(fromPos))
            return False    
        if toPos not in range(9):
            logger.debug("toPos[0〜8]以外を指定した [{}]".format(toPos))
            return False    
        if fromPos == toPos:
            logger.debug("同じ位置への移動を指定した [{}->{}]".format(fromPos,toPos))
            return False

        # fromPosの最大のコマが自分のコマかどうかの確認
        if self.getOwner(fromPos) != side:
            logger.debug("所持していない位置からは移動できない [{} : {}]".format(fromPos,self.getOwner(fromPos)))
            return False
    ...
    def judge(self):
        ...
        if self.__winner == None:
            ...
            # self.render('owner') 
            if self.__enableRender:           
                logger.info("Winner is side {}".format(self.__winner))
            return True
